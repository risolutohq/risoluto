...
/**
 * Walk all targets and sources; cluster by the `ideas:` frontmatter tag.
 * @returns {Map<string, Set<string>>} concept -> set of target slugs that evidence it
 */
...
function buildConceptNote(concept, targets) {
  const wikilinks = [...targets]
    .sort()
    .map((t) => `- [[targets/${t}]]`)
    .join("\n");
...
async function writeWikiNotes(conceptMap) {
  ...
  for (const [concept, targets] of conceptMap) {
    ...
    const content = buildConceptNote(concept, targets);
...
/**
 * Append one idea row per concept. Returns a summary of actions.
 * @returns {{ concept: string, action: string }[]}
 */
function appendRoadmapRows(conceptMap) {
...
async function main() {
  ...
  const conceptMap = await buildConceptMap();
  ...
  const wikiResults = await writeWikiNotes(conceptMap);
  ...
  const roadmapResults = appendRoadmapRows(conceptMap);
