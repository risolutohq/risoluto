...
6. **Reconcile the issue's acceptance criteria** (save-issue operation, editing the `description` — leave status untouched). For each line in the `## Acceptance criteria` list:
   ...
   - **Reachability gate (operator-invocable capabilities, in-loop).** For any criterion whose behaviour an operator invokes (a CLI command, an HTTP/webhook request, or a Slack action), ticking requires all three — verified in-loop, not just by CI: (1) a `src/reachability/capability-manifest.json` entry exists for the capability, (2) `pnpm reach:check` is green for that entry, AND (3) at least one e2e or integration test drives it from the real intake adapter — not via an internal call with a stubbed entry point. If any of the three is absent, leave the box unchecked and append `(deferred: missing manifest entry / reach:check / intake e2e)` so the gap is explicit.
