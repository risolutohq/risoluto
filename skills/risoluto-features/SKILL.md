...
description: Update the Risoluto Feature Spine — a canonical, code-backed inventory of every user-observable and backend-surface feature Risoluto ships. Use whenever Omer says /risoluto-features, asks to "update the spine", "regenerate risoluto features", "check what's new in risoluto", "diff the risoluto spine", "refresh feature inventory", or "rebuild RISOLUTO_FEATURES". Operates on a TWO-REPO model — source code lives in `risolutohq/risoluto` (cloned to `.spine-workspace/source/`), the spine itself lives in the `risolutohq/risoluto-research` submodule at `research/`. Re-verifies every existing entry against current source, detects net-new features via git log + closed GitHub issues + new exports + docs scan, evolves "Needs follow-up" and "Analyst notes" instead of regenerating them, auto-derives Summary and Coverage, runs fact-check + markdown lint, and uses AskUserQuestion as a hard commit gate. Map-reduce architecture: per-module subagents do feature extraction in parallel so the main agent stays light.
---
...
| **Storage** | `risolutohq/risoluto-research` | `research/` (submodule of time-aura) | The spine files themselves (`RISOLUTO_FEATURES.md/.json/.html`). NO source code. |
...
|---|---|---|
| In a time-aura worktree | `git rev-parse --show-toplevel` ends in `time-aura` | Tell Omer to `cd` into the time-aura worktree. |
...
| `.spine-workspace/` is gitignored | `git check-ignore .spine-workspace/source 2>/dev/null` returns the path | If not, append `/.spine-workspace/` to time-aura's root `.gitignore` and stage it. |
...
python3 ~/.claude/skills/risoluto-features/scripts/render_meta.py \
...
```bash
python3 ~/.claude/skills/risoluto-features/scripts/diff_spines.py \
...
python3 ~/.claude/skills/risoluto-features/scripts/validate_json.py \
...
python3 ~/.claude/skills/risoluto-features/scripts/fact_check.py \
  "$STORAGE_DIR/RISOLUTO_FEATURES.json.next" \
...
python3 ~/.claude/skills/risoluto-features/scripts/lint_md.py \
...
```bash
python3 ~/.claude/skills/risoluto-features/scripts/render_html.py \
...
- `render_meta.py` — Summary table + Coverage manifest from JSON
