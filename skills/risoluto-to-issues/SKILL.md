...
<!-- Each criterion is a falsifiable BEHAVIOURAL assertion — the spec for a failing test /risoluto-tdd will write, e.g. "a Workflow Run failing at step 3 replays from 3, not 0". NOT the global gate (build/lint/test/typecheck/coverage); every merge enforces that already. For any operator-invocable capability this slice introduces, also include: "Reachable from <adapter> — src/reachability/capability-manifest.json entry exists, pnpm reach:check green, proven by an e2e at the real intake adapter." -->
...
- **Capability-bearing slices require a reachability AC.** Every slice that introduces an operator-invocable capability must carry at least one reachability criterion of the form: "Reachable from `<adapter>` — a `src/reachability/capability-manifest.json` entry exists, `pnpm reach:check` is green, and an e2e drives it from the real intake adapter." This criterion is not optional and not implied by passing the gate. A slice whose ACs omit its reachability path is not ready to start.
