---
name: risoluto-vault
description: 'Configure the `research/` submodule as a scoped Obsidian vault for the Risoluto planning pipeline. Use when Omer says /risoluto-vault, "set up the research vault", "configure obsidian on research", "repair vault config", "reinstall vault templates", "vault is drifting", "pin vault plugins", or similar. Writes canonical `.obsidian/`, `templates/`, and Dataview view files, pins the Web Clipper / Dataview / Templater plugin set, and keeps relative markdown links. Idempotent; re-runs repair managed files and preserve operator-owned preferences.'
---
