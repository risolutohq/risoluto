...
const workflowRunEventRecordSchema = z
  .object({
    at: z.string(),
    eventType: z.string(),
...
const workflowRunAttemptSummarySchema = z.object({
  id: z.string(),
  workflowRunId: z.string(),
  attemptNumber: z.number().optional(),
  reason: z.enum(["initial", "retry", "resume"]).optional(),
...
/** GET /api/v1/workflow-runs — Workflow Run list projection. */
...
export const workflowRunDetailResponseSchema = z.object({
  type: z.literal("workflow_run.loaded"),
  workflowRun: workflowRunRecordSchema,
});
...
/** GET /api/v1/workflow-runs/:workflow_run_id/events — Workflow Run event-log projection. */
export const workflowRunEventsListResponseSchema = z.object({
  type: z.literal("workflow_run.events_listed"),
  workflowRun: workflowRunRecordSchema,
  events: z.array(workflowRunEventRecordSchema),
});

/** GET /api/v1/workflow-runs/:workflow_run_id/run-attempts — Run Attempt history projection. */
export const workflowRunAttemptsListResponseSchema = z.object({
  type: z.literal("workflow_run.run_attempts_listed"),
  workflowRun: workflowRunRecordSchema,
  runAttempts: z.array(workflowRunAttemptSummarySchema),
});
