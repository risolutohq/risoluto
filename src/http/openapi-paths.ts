...
export function buildWorkflowRunPaths(): Record<string, PathItem> {
  return {
    "/api/v1/workflow-runs": workflowRunsListPath(),
    "/api/v1/workflow-runs/{workflow_run_id}": workflowRunDetailPath(),
    "/api/v1/workflow-runs/{workflow_run_id}/events": workflowRunEventsPath(),
    "/api/v1/workflow-runs/{workflow_run_id}/run-attempts": workflowRunAttemptsPath(),
  };
}

function workflowRunsListPath(): PathItem {
  return {
    get: {
      tags: ["Workflow Runs"],
...
function workflowRunDetailPath(): PathItem {
  return {
    get: {
      tags: ["Workflow Runs"],
      ...
      security: protectedReadSecurity,
      parameters: [pathParam("workflow_run_id", "Workflow Run identifier")],
      responses: {
        ...protectedReadResponses("Workflow Run metadata projection", toSchema(workflowRunDetailResponseSchema)),
        "404": errorResponse("Workflow Run not found"),
        "503": errorResponse("Workflow Run archive not configured"),
      },
    },
  };
...
function workflowRunEventsPath(): PathItem {
  return {
    get: {
      tags: ["Workflow Runs"],
      summary: "List Workflow Run event-log entries",
      operationId: "listWorkflowRunEvents",
      security: protectedReadSecurity,
      parameters: [pathParam("workflow_run_id", "Workflow Run identifier")],
      responses: {
        ...protectedReadResponses("Workflow Run event-log projection", toSchema(workflowRunEventsListResponseSchema)),
        "404": errorResponse("Workflow Run not found"),
        "503": errorResponse("Workflow Run archive not configured"),
      },
    },
...
function workflowRunAttemptsPath(): PathItem {
  return {
    get: {
      tags: ["Workflow Runs"],
      summary: "List Run Attempts for a Workflow Run",
      operationId: "listWorkflowRunAttempts",
      security: protectedReadSecurity,
      parameters: [pathParam("workflow_run_id", "Workflow Run identifier")],
      responses: {
        ...protectedReadResponses(
          "Workflow Run attempt history projection",
          toSchema(workflowRunAttemptsListResponseSchema),
        ),
        "404": errorResponse("Workflow Run not found"),
        "503": errorResponse("Workflow Run archive not configured"),
      },
