...
const CONTINUATION_PROMPT_PARTS = [
  "Continue the current Workflow Run, make concrete progress, and stop only when done or blocked.",
  "When the Workflow Run is complete, end your final message with `RISOLUTO_STATUS: DONE`.",
...
function emitTurnCompletedEvent(
  ...
  input.runInput.onEvent({
    ...
    metadata: input.runInput.workflowRun ? { workflowRun: input.runInput.workflowRun } : undefined,
  });
}
...
async function runSingleTurn(
  ...
  const turnResult = await input.connection.request(CODEX_METHOD.TurnStart, {
    ...
    title: `Workflow Run ${workflowRun?.identifier ?? input.runInput.issue.identifier}: ${
      workflowRun?.title ?? input.runInput.issue.title
    }`,
...
async function handleTurnLoop(
  ...
  if (state.turnCount >= input.config.agent.maxTurns) {
    ...
    // would be classified as a normal exit) (RIS-257).
    return {
      kind: "failed",
