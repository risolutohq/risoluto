...
import { tryHandleWorkflowRunCommand } from "./workflow-run-command.js";
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  const workflowRunCommandExitCode = await tryHandleWorkflowRunCommand(argv);
  if (workflowRunCommandExitCode !== null) {
    return workflowRunCommandExitCode;
  }
