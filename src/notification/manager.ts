...
export class NotificationManager {
  ...
  async notify(
    ...
    // the retry of the same notification within the window (RIS-264).
...
function defaultTitle(type: NotificationEvent["type"]): string {
  switch (type) {
    case "workflow_run_claimed":
      return "Workflow Run claimed";
    case "workflow_run_worker_launched":
      return "Workflow Run worker launched";
    case "issue_claimed":
