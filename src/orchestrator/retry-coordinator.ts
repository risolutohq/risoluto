...
class RetryCoordinatorImpl implements RetryCoordinator {
  ...
  private async handleRetryLaunchFailure(issue: Issue, attempt: number, error: unknown): Promise<void> {
    ...
    this.runtime.notify({
      ...
      severity: "critical",
      timestamp: nowIso(),
      message: `Workflow Run retry startup failed: ${errorText}`,
      issue: issueRef(issue),
      attempt,
      metadata: {
        error: errorText,
  ...
  private async handleContinuationExhausted(ctx: OutcomeContext, prepared: PreparedWorkerOutcome): Promise<void> {
    ...
    ctx.notify({
      ...
      metadata: {
        ...
        workflowRun: workflowRunRef(latestIssue),
      },
    });
    ...
    emitWorkflowRunFailed(ctx, latestIssue, attempt);
...
function emitWorkflowRunFailed(ctx: OutcomeContext, issue: Issue, attempt: number | null): void {
