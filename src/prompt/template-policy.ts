...
const TEMPLATE_REFERENCE_RE =
  /^(attempt|issue(?:\.[A-Za-z_]\w*)+|workflowRun(?:\.[A-Za-z_]\w*)+|workspace(?:\.[A-Za-z_]\w*)+)$/;
const OUTPUT_EXPR_RE = TEMPLATE_REFERENCE_RE;
const IF_STATEMENT_RE =
  /^if\s+(attempt|issue(?:\.[A-Za-z_]\w*)+|workflowRun(?:\.[A-Za-z_]\w*)+|workspace(?:\.[A-Za-z_]\w*)+)$/;
  ...
  if (!OUTPUT_EXPR_RE.test(expression)) {
    throw new PromptTemplateValidationError(
      `unsupported Liquid output expression: ${expression || "(empty)"}. ` +
        "Only direct workflowRun.*, issue.*, workspace.*, and attempt references are allowed.",
    );
  }
}
