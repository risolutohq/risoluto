...
Flaky tests with an open ticket may be parked in `quarantine.json` (currently empty). Quarantine is
for genuinely flaky tests with a tracked fix — never a place to silence a test an agent could not fix.
Adding an entry requires the open ticket reference.
