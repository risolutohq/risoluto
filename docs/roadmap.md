...
| --- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ----------------------------------------------------------------------------- | ------------- |
| 1   | Workflow-first AFK MVP <!-- slug:workflow-first-afk-mvp -->   | Proves Risoluto's core thesis: configurable Workflow Runs execute AFK engineering work end to end across CLI, Slack, HTTP, tracker intake, worktrees, PR/CI, verifier, evidence, memory, and handoff without making Linear/GitHub issue identity primary.                                                                                                                                                                                                                                                                                                                                                                                                                              | L    | shipped                                                                       | —             |
| 2   | Verification ladder <!-- slug:verification-ladder -->         | The green gate proves units work, not that capabilities are reachable from a real intake — workflow-first-afk-mvp shipped features "done" yet wired only in tests, forcing a costly after-the-fact rework pass. A static reachability gate + e2e intake tier + in-loop pipeline enforcement makes "done" mean shipped, not just green.                                                                                                                                                                                                                                                                                                                                                 | M    | shipped                                                                       | —             |
| 3   | Code review remediation <!-- slug:code-review-remediation --> | A whole-project deep review (CODE-REVIEW-FINDINGS.md) found 88 issues across all 32 `src/` modules — 2 Critical (agent/codex auto-approval of privileged actions; orphan worker surviving shutdown), ~35 High (pervasive secret leakage, SSRF, option/path injection, prototype pollution, non-atomic persistence, `withRetry` swallowing final errors), plus ~40 Medium / ~11 Low. These are latent security and reliability defects in shipped surface; fixing them now hardens the AFK core before more surface layers on top.                                                                                                                                                      | L    | shipped                                                                       | —             |
| 4   | AFK orchestrator <!-- slug:afk-orchestrator -->               | The current AFK path (/risoluto-goal-run + conductor.workflow.mjs) needs a pre-rendered goal package and threads merges and per-wave gates through the operator; it cannot react to live opencode session events or recover from a crash mid-cascade. A standing scripts/afk-orchestrator/ that reads the Linear blocked-by DAG directly, drives opencode 'fixer' sessions in per-slice git worktrees over SSE, serializes merges into integration/<slug> behind the exact 7-command v1 gate, and owns Linear status plus proof-only AC writeback would make a multi-wave PRD cascade to merged unattended with crash recovery — turning AFK from a guided run into an autonomous one. | L    | [building](https://linear.app/ninetech/project/afk-orchestrator-e0ea11426d15) | —             |
