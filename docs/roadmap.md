...
| 3   | Code review remediation <!-- slug:code-review-remediation --> | A whole-project deep review (CODE-REVIEW-FINDINGS.md) found 88 issues across all 33 `src/` modules — 2 Critical (agent/codex auto-approval of privileged actions; orphan worker surviving shutdown), ~35 High (pervasive secret leakage, SSRF, option/path injection, prototype pollution, non-atomic persistence, `withRetry` swallowing final errors), plus ~40 Medium / ~11 Low. These are latent security and reliability defects in shipped surface; fixing them now hardens the AFK core before more surface layers on top.                                                                                                                                                      | L    | [building](https://linear.app/ninetech/project/code-review-remediation-53f1ca436217) | —             |
| 4   | AFK orchestrator <!-- slug:afk-orchestrator -->               | The current AFK path (/risoluto-goal-run + conductor.workflow.mjs) needs a pre-rendered goal package and threads merges and per-wave gates through the operator; it cannot react to live opencode session events or recover from a crash mid-cascade. A standing scripts/afk-orchestrator/ that reads the Linear blocked-by DAG directly, drives opencode 'fixer' sessions in per-slice git worktrees over SSE, serializes merges into integration/<slug> behind the exact 6-command v1 gate, and owns Linear status plus proof-only AC writeback would make a multi-wave PRD cascade to merged unattended with crash recovery — turning AFK from a guided run into an autonomous one. | L    | [building](https://linear.app/kyanite/project/afk-orchestrator-77742470e134)         | —             |
