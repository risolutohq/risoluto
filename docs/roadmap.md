...
| 3   | Code review remediation <!-- slug:code-review-remediation --> | A whole-project deep review (CODE-REVIEW-FINDINGS.md) found 88 issues across all 33 `src/` modules — 2 Critical (agent/codex auto-approval of privileged actions; orphan worker surviving shutdown), ~35 High (pervasive secret leakage, SSRF, option/path injection, prototype pollution, non-atomic persistence, `withRetry` swallowing final errors), plus ~40 Medium / ~11 Low. These are latent security and reliability defects in shipped surface; fixing them now hardens the AFK core before more surface layers on top. | L    | next                                                                                | —             |
