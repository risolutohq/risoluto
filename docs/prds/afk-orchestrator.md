...
**DAG source (D5).** `dag.ts` calls Linear GraphQL directly (`LINEAR_API_KEY`) for `from:prd-<slug>` issues and their `blockedBy`; waves by dependency depth (port the `readyIn` logic from the existing `conductor.workflow.mjs`); cycles throw. **Closed-world construction (fail closed):** a `from:prd-<slug>` issue blocked by an issue *outside* the selected slug throws unless that blocker is explicitly marked external/satisfied (a `dep:external` label or a resolved Done/Canceled/Duplicate state); `canceled`/`duplicate`/`archived` blockers are treated as satisfied (excluded from wave depth) and logged; an unlabelled or cross-project blocker that is neither satisfied nor marked external fails the build rather than silently producing a false wave order.
...
**Permission safety (D4).** Honour `CONTROL.md require_approval_for`: auto-allow only non-listed types; a listed type journals `awaiting-approval` and holds (never blanket-allow). The **minimal fail-closed parking ships with the Wave-1 single-slice coder runner** — before any unattended session runs, a listed (or, until the allowlist config lands, any unrecognized) permission type parks the slice rather than self-authorizing, so the safety invariant precedes the capability. The fuller `CONTROL.md` steering surface (pause-at-boundary, skip-wave, `session.abort`) stays Wave 3; only the deny/park default moves early.

**Budgets (D9).** Config-driven max cascade wall-clock and max total sessions (coder/reviewer/diagnoser spawns counted); breach → halt + `aborted-budget`. Beyond the global caps, per-session/per-slice bounds stop one expensive session from burning the whole budget: a **per-session max wall-clock** (a session past it is aborted and its slice journaled), a **max diagnoser invocations per slice**, plus the per-slice caps that already exist (2 idle-no-commit → `blocked` [US6]; 3 failed reviews → `rejected` [US21]; merge rebase-retry cap → `blocked` [US8]). The halt report carries a **budget summary**: wall-clock used, sessions spawned per role, and the breached cap.
...
**Journal durability (D13).** The append-only JSONL journal is the crash-recovery substrate, so its semantics are pinned rather than "atomic enough": every event carries a **schema version** and a **monotonic sequence id** (a deterministic event id); appends use `O_APPEND` with an explicit flush and an `fsync` on the SIGTERM / wave-boundary flush path, so a crash mid-write can only leave a **torn trailing line**, which the reader discards on load; the **reducer is idempotent** — replaying a duplicate event id is a no-op, so a boot/reconnect replay never double-counts; an unparseable *non-trailing* line **fails closed** with its byte offset; and a schema-version bump carries a forward-only migration note. These are the guarantees US18 (crash resume) and US20 (reconcile) depend on.
