...
| All persistence / dispatch / scheduling / observability key off run identity | ⚠ Drifted     | SQLite tables key on `attempt_id` / `issue_id`; **no `workflow_run_id` column** (`persistence/sqlite/schema.ts:17-81`); dispatch falls back to `issue.id` (`dispatch/server.ts:85`)                                                                                                                                                                                                                                                                                                          |
...
| Outer transitions are enforced                            | Delivered | `canTransition` (`state/machine.ts:155`) / `assertTransition` (`state/machine.ts:162`), `assertWorkflowStateTransition` (`state/topology.ts:55`), `http/transition-handler.ts:21`                                                                                                                                                            |
...
| Events carry a monotonic sequence                       | Delivered     | `archive.ts:216` computes `Math.max(0, ...sequences) + 1` in `nextWorkflowRunEventSequenceForRunDir`                                                 |
...
| A network-shaped contract (HTTP + SSE) exists              | Delivered     | `dispatch/client.ts:132` (`fetch`; `text/event-stream` header at `:137`); `dispatch/server.ts` `createDataPlaneServer` (Express), launched by `dispatch/entrypoint.ts`                                                                    |
| Control plane owns CLI + HTTP operator surfaces            | Delivered     | `cli/`, `http/server.ts` (full route + OpenAPI surface)                                                                                                                                                                                   |
| The interface is network-shaped by default, not in-process | ⚠ Drifted     | `DISPATCH_MODE` defaults to `"local"` → in-process call (`dispatch/factory.ts:34`). Default deployment is a monolith                                                                                                                      |
...
| A pre-push hook detects PRD drift and blocks the push         | Removed   | Drift gate retired — the `.husky/pre-push` drift block, `scripts/prd-drift-check.ts`, and `.github/workflows/prd-drift.yml` were deleted. Git-canonical PRDs are now a convention; Linear is hand-editable and reconciled via `/risoluto-to-prd` or `pnpm prd:reconcile`.                                                                                                                       |
| The drift check requires `LINEAR_API_KEY`                     | Removed   | n/a — drift check removed (see row above)                                                                                                                                                                                                                                                                                                                                                       |
...
| Skills are wired via symlink into `~/.claude/skills`          | Partial   | 20 symlinks in repo-local `.claude/skills/` → `../../skills/risoluto-*`, discovered via project-level skill loading; not symlinked into the user-global `~/.claude/skills/`                                                                                                                                                                                                                     |
| The pipeline has been dogfooded end to end                    | Partial   | Four PRDs exist (`docs/prds/workflow-first-afk-mvp.md`, `docs/prds/verification-ladder.md`, `docs/prds/afk-orchestrator.md`, `docs/prds/code-review-remediation.md`); Phase 4 (`to-issues` → `tdd`) has been exercised for `workflow-first-afk-mvp` (issues labelled `from:prd-workflow-first-afk-mvp`, code merged), but a full research→merge cycle (Modes A + B) has not been run end to end |
...
pipeline is operator-driven (explicit invocation per phase) and the slice graph is
non-deterministic.
