...
const issueIdentityFieldNames = new Set(["id", "identifier", "title", "state"]);

const missingIssueIdentityValue = fc.oneof(
  fc.string(),
  fc.integer(),
  fc.double(),
  fc.boolean(),
  ...
  fc.array(fc.string()),
  fc.dictionary(
    fc.string().filter((key) => !issueIdentityFieldNames.has(key)),
    fc.string(),
  ),
);
...
describe("property: normalizeIssue — fallback defaults", () => {
  it("missing id, identifier, title, and state fall back to safe defaults", () => {
    fc.assert(
      fc.property(missingIssueIdentityValue, (garbage) => {
