...
describe("executeTurns", () => {
  ...
  it("uses Workflow Run continuation prompt for subsequent turns", async () => {
    ...
    expect(secondCall[1].input[0].text).toBe(
      "Continue the current Workflow Run, make concrete progress, and stop only when done or blocked. When the Workflow Run is complete, end your final message with `RISOLUTO_STATUS: DONE`. If you are blocked and cannot proceed, end your final message with `RISOLUTO_STATUS: BLOCKED`.",
    );
  });
  ...
  it("passes Workflow Run title to turn/start", async () => {
    vi.mocked(isActiveState).mockReturnValueOnce(false);
    ...
    const requestMock = (input.connection as unknown as { request: ReturnType<typeof vi.fn> }).request;
    const params = requestMock.mock.calls[0][1];
    expect(params.title).toBe("Workflow Run MT-1: Test");
  });

  it("prefers explicit Workflow Run title for turn/start", async () => {
    vi.mocked(isActiveState).mockReturnValueOnce(false);

    const input = makeInput();
    input.runInput.issue = {
      id: "legacy-issue-1",
      identifier: "LEG-1",
      title: "Legacy issue title",
      state: "In Progress",
    };
    input.runInput.workflowRun = {
      id: "workflow-run-1",
      identifier: "WR-1",
    ...
    const requestMock = (input.connection as unknown as { request: ReturnType<typeof vi.fn> }).request;
    const params = requestMock.mock.calls[0][1];
    expect(params.title).toBe("Workflow Run WR-1: Workflow Run title");
  });

  it("emits explicit Workflow Run metadata on turn completion events", async () => {
    vi.mocked(isActiveState).mockReturnValueOnce(false);
    ...
    expect(input.runInput.onEvent).toHaveBeenCalledWith(
