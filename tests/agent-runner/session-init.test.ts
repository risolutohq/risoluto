...
describe("initializeSession", () => {
  ...
  describe("happy path", () => {
    ...
    it("renders Workflow Run prompt context for model-facing templates", async () => {
      const session = makeMockSession();
      session.connection.request
      ...
      const input = makeInput({
        promptTemplate: "Workflow Run {{ workflowRun.identifier }}: {{ workflowRun.title }}",
      });
      ...
      expect(result).toEqual({ threadId: "thread-abc", prompt: "Workflow Run MT-42: Test Issue" });
    });

    it("prefers explicit Workflow Run prompt context over compatibility issue fields", async () => {
      const session = makeMockSession();
      session.connection.request
        .mockResolvedValueOnce({})
        .mockResolvedValueOnce({ status: "authenticated" })
      ...
      const input = makeInput({
        issue: createIssue({ id: "legacy-issue-1", identifier: "LEG-1", title: "Legacy issue title" }),
        workflowRun: {
          id: "workflow-run-1",
          identifier: "WR-1",
