...
describe("createServices", () => {
  ...
  it("wires Linear webhook intake to Workflow Run artifacts under archiveDir", async () => {
    await createServices(
      {
        getConfig: vi.fn().mockReturnValue({
          webhook: {
    ...
    const handlerDeps = mockHttpServer.mock.calls[0][0].webhookHandlerDeps;
    await handlerDeps.acceptLinearTriggeredWorkflowRun({
      action: "create",
    ...
    expect(mockAcceptLinearTriggeredWorkflowRun).toHaveBeenCalledWith({
      archiveDir,
      action: "create",
      deliveryId: "linear-delivery-1",
      issue: {
