...
describe("DispatchClient", () => {
  ...
  it("includes Workflow Run reference in the dispatch request body", async () => {
    const mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      body: {
        getReader: () => ({
    ...
    const dispatchBody = JSON.parse(String(mockFetch.mock.calls[0][1]?.body)) as Record<string, unknown>;
    expect(dispatchBody.workflowRun).toEqual({
      id: "test-id",
      identifier: "TEST-1",
      title: "Test Issue",
      url: "https://linear.app/test/TEST-1",
