...
describe("RetryCoordinator dispatch", () => {
  ...
  it("marks continuation exhaustion as failed without queuing a retry", async () => {
    ...
    const workflowIssue = createIssue({
      id: "workflow-run-continuation-exhausted",
      identifier: "WR-CONTINUATION",
      title: "Workflow Run continuation title",
      description: "Run this continuation workflow",
      url: "https://linear.app/test/WR-CONTINUATION",
    });
    const workflowEntry = createRunningEntry({ issue: workflowIssue, sessionId: "session-continued" });

    await limitedHarness.ctx.retryCoordinator.dispatch(
      limitedHarness.ctx,
      makePrepared(makeOutcome(), workflowEntry, workflowIssue, workspace, modelSelection, 1),
    );

    expect(limitedHarness.retryEntries.size).toBe(0);
    expect(limitedHarness.releaseIssueClaim).toHaveBeenCalledWith(workflowIssue.id);
    expect(limitedHarness.notify).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "worker_failed",
        message: expect.stringContaining("1 continuations"),
    ...
    expect(limitedHarness.completedViews.get(workflowIssue.identifier)).toMatchObject({
      status: "failed",
      error: "max_continuations_exceeded",
    });
    expect(limitedHarness.ctx.deps.eventBus?.emit).toHaveBeenCalledWith("workflow_run.completed", {
      workflowRun: {
        id: "workflow-run-continuation-exhausted",
        identifier: "WR-CONTINUATION",
        title: "Workflow Run continuation title",
...
describe("RetryCoordinator timer flows", () => {
  ...
  it("persists retry launch failures and records a failed view", async () => {
    ...
    const harness = makeHarness();
    const issue = createIssue({
      id: "workflow-run-retry-launch-failure",
      identifier: "WR-RETRY-LAUNCH",
      title: "Workflow Run retry launch title",
      description: "Run this retry launch workflow",
      url: "https://linear.app/test/WR-RETRY-LAUNCH",
    });
    ...
    expect(harness.notify).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "worker_failed",
        message: "Workflow Run retry startup failed: spawn failed",
        metadata: expect.objectContaining({
          error: "spawn failed",
