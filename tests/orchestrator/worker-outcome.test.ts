...
describe("handleWorkerOutcome - service stopped path", () => {
  it("notifies service-stopped cancellation with Workflow Run identity", async () => {
    const issue = makeIssue({
      id: "workflow-run-service-stopped",
      identifier: "WR-STOP",
      title: "Workflow Run service stop title",
      description: "Run interrupted by service stop",
      url: "https://linear.app/test/WR-STOP",
    });
    const ctx = makeCtx({ isRunning: false, latestIssue: issue });
    const entry = makeEntry({ issue });
    ctx.runningEntries.set("workflow-run-service-stopped", entry);
    ...
    expect(ctx.releaseIssueClaim).toHaveBeenCalledWith("workflow-run-service-stopped");
    ...
    expect(ctx.deps.eventBus?.emit).toHaveBeenCalledWith("workflow_run.completed", {
      workflowRun: {
        id: "workflow-run-service-stopped",
        identifier: "WR-STOP",
        title: "Workflow Run service stop title",
...
describe("handleWorkerOutcome - terminal state path", () => {
  it("emits Workflow Run completion when terminal state cleanup runs", async () => {
    const issue = makeIssue({
      id: "workflow-run-terminal",
      identifier: "WR-DONE",
      title: "Workflow Run terminal title",
      description: "Run reached terminal cleanup",
      state: "Done",
      url: "https://linear.app/test/WR-DONE",
    });
    const ctx = makeCtx({ latestIssue: issue });
    const entry = makeEntry({ issue });
    ctx.runningEntries.set("workflow-run-terminal", entry);

    await handleWorkerOutcome(ctx, makeOutcome(), entry, issue, makeWorkspace(), 1);

    expect(ctx.deps.workspaceManager.removeWorkspace).toHaveBeenCalledWith(
      "WR-DONE",
      expect.objectContaining({ identifier: "WR-DONE", state: "Done" }),
    );
    expect(ctx.releaseIssueClaim).toHaveBeenCalledWith("workflow-run-terminal");
...
describe("handleWorkerOutcome - inactive (non-terminal) state", () => {
  it("emits Workflow Run completion when the run becomes inactive", async () => {
    const issue = makeIssue({
      id: "workflow-run-paused",
      identifier: "WR-PAUSE",
      title: "Workflow Run inactive title",
      description: "Run became inactive",
      state: "Backlog",
      url: "https://linear.app/test/WR-PAUSE",
    });
    const ctx = makeCtx({ latestIssue: issue });
    const entry = makeEntry({ issue });
    ctx.runningEntries.set("workflow-run-paused", entry);

    await handleWorkerOutcome(ctx, makeOutcome(), entry, issue, makeWorkspace(), 1);

    expect(ctx.deps.workspaceManager.removeWorkspace).not.toHaveBeenCalled();
    expect(ctx.releaseIssueClaim).toHaveBeenCalledWith("workflow-run-paused");
    expect(ctx.retryEntries.size).toBe(0);
    const view = getCompletedView(ctx, "WR-PAUSE");
...
describe("handleWorkerOutcome - hard failure path", () => {
  ...
  it("notifies operator cancellation with Workflow Run identity", async () => {
    const issue = makeIssue({
      id: "workflow-run-cancel",
      identifier: "WR-CANCEL",
      title: "Workflow Run cancellation title",
      description: "Run this cancellation workflow",
      url: "https://linear.app/test/WR-CANCEL",
    });
    const ctx = makeCtx({ latestIssue: issue });
    const entry = makeEntry({ issue });
    ctx.runningEntries.set("workflow-run-cancel", entry);
  ...
  it("does not retry for hard failure codes like startup_failed", async () => {
    const issue = makeIssue({
      id: "workflow-run-2",
      identifier: "WR-2",
      title: "Workflow Run failure title",
      description: "Run this failure workflow",
      url: "https://linear.app/test/WR-2",
    });
    const ctx = makeCtx({ latestIssue: issue });
    const entry = makeEntry({ issue });
    ctx.runningEntries.set("workflow-run-2", entry);
...
describe("handleWorkerOutcome - stop signal detection", () => {
  it("marks done and triggers git post-run when RISOLUTO_STATUS: DONE detected", async () => {
    ...
    ctx.runningEntries.set("workflow-run-1", entry);
    ...
    const view = getCompletedView(ctx, "WR-1");
...
describe("handleWorkerOutcome - smart retry routing via codexErrorInfo", () => {
  ...
  it("routes RateLimited to retry with 30s default delay", async () => {
    const issue = makeIssue({
      id: "workflow-run-retry",
      identifier: "WR-RETRY",
      title: "Workflow Run retry title",
      description: "Run this retry workflow",
      url: "https://linear.app/test/WR-RETRY",
    });
    const ctx = makeCtx({ latestIssue: issue });
    const entry = makeEntry({ issue });
    ctx.runningEntries.set("workflow-run-retry", entry);
    const before = Date.now();
    ...
    const retryEntry = expectRetryDelay(ctx, before, 30_000, "workflow-run-retry");
    expect(retryEntry.attempt).toBe(2);
