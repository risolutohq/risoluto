...
describe("SSE handler", () => {
  ...
  let baseUrl: string;
  const workflowRunStartedPayload: RisolutoEventMap["workflow_run.started"] = {
    workflowRun: {
      id: "wr-1",
      identifier: "WR-1",
      title: "Run operator workflow",
      description: "Start the workflow worker",
      url: null,
    },
    attempt: 1,
  };
  ...
  it("forwards EventBus events to the response", async () => {
    const lines = await collectDataLines(baseUrl, 2, () => {
      eventBus.emit("workflow_run.started", workflowRunStartedPayload);
    });
