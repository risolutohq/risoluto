 ...
 *
 * Covers: initial connected event, every RisolutoEventMap event type,
...
const samplePayloads: { [K in keyof RisolutoEventMap]: RisolutoEventMap[K] } = {
  "workflow_run.accepted": {
    workflowRunId: "wr_accepted",
    source: "cli",
    title: "Classify incoming operator intent",
  ...
  "workflow_run.started": {
    workflowRun: {
      id: "wr_started",
      identifier: "WR-RUN",
      title: "Run operator workflow",
      description: "Launch the workflow worker",
      url: "https://linear.app/test/WR-RUN",
    },
    attempt: 1,
  },
  "workflow_run.queued": {
  ...
  "workflow_run.stalled": {
    workflowRunId: "wr_stalled",
    identifier: "WR-STALLED",
    reason: "no worker heartbeat",
  },
  "workflow_run.worker_failed": {
    workflowRunId: "wr_worker_failed",
    identifier: "WR-WORKER-FAILED",
    error: "worker process exited unexpectedly",
  },
  "workflow_run.workspace_event": {
    workflowRunId: "wr_workspace",
    identifier: "WR-WORKSPACE",
    status: "ready",
  },
  "workflow_run.agent_event": {
    workflowRunId: "wr_agent",
    identifier: "WR-AGENT",
    type: "turn_completed",
  ...
  "workflow_run.completed": {
    workflowRun: {
      id: "wr_completed",
      identifier: "WR-DONE",
      title: "Complete operator workflow",
      description: "Run this workflow to completion",
...
describe("SSE contract tests (full HttpServer stack)", () => {
  ...
  it("delivers multiple event types in emission order", async () => {
    ...
    const emitOrder: Array<keyof RisolutoEventMap> = [
      "workflow_run.started",
      "poll.complete",
      "workflow_run.completed",
    ];
  ...
  it("does not throw when emitting before any SSE client connects", async () => {
    ...
    expect(() => {
      ctx.eventBus!.emit("workflow_run.started", samplePayloads["workflow_run.started"]);
    }).not.toThrow();
  ...
  it("broadcasts the same event to two concurrent SSE clients", async () => {
    ...
    const payload = samplePayloads["workflow_run.started"];
  ...
  // ---- All event types in a single stream ----

  it("delivers all event types through a single SSE connection", async () => {
    ctx = await startTestServer({ eventBus: true });

    const expectedFrameCount = allEventTypes.length + 1;
    const frames = await collectFrames(ctx.baseUrl, expectedFrameCount, () => {
