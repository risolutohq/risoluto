...
describe("OpenAPI Contract Tests", () => {
  ...
  describe("Workflow Runs", () => {
    it("support endpoints match their OpenAPI response schemas", async () => {
      const workflowRun = createWorkflowRunRecord({
        archiveDir: ctx.dataDir,
        title: "Validate support HTTP contracts",
        intent: "Prove Workflow Run support endpoints match the runtime OpenAPI spec.",
        source: "cli",
      ...
      const checks = [
        {
          ...
          context: "GET /api/v1/workflow-runs 200",
        },
        {
          specPath: "/api/v1/workflow-runs/{workflow_run_id}",
          path: `/api/v1/workflow-runs/${workflowRun.id}`,
          context: "GET /api/v1/workflow-runs/{workflow_run_id} 200",
        },
        {
          specPath: "/api/v1/workflow-runs/{workflow_run_id}/events",
          path: `/api/v1/workflow-runs/${workflowRun.id}/events`,
          context: "GET /api/v1/workflow-runs/{workflow_run_id}/events 200",
        },
        {
          specPath: "/api/v1/workflow-runs/{workflow_run_id}/run-attempts",
          path: `/api/v1/workflow-runs/${workflowRun.id}/run-attempts`,
          context: "GET /api/v1/workflow-runs/{workflow_run_id}/run-attempts 200",
        },
      ];

      for (const check of checks) {
        const validate = compileResponseSchema(check.specPath, "get", "200");
        const response = await fetchApi(check.path);

        expect(response.status).toBe(200);
        expect(response.headers.get("content-type")).toContain("application/json");
        expectSchemaValid(validate, await response.json(), check.context);
      }
    });
  });
