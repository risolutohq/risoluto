...
| B3 | Keep `anvil.config.yaml`? | **Yes** (quarantine) — anvil is in the active skill toolkit |
| B4 | Keep `opencode.json`, `agent-browser.json`? | **No** — both deleted |
| B5 | `.agents/` contents? | **Keep `PLANS.md` only**; drop `archive/`, `prompts/`, `workflows/`, `skills` symlink |
